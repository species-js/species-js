...
### Q.001 — `getDefinedConstructorName` direct-access vs. descriptor read (deferred 2026-06-03)

The current impl uses `const { name } = constructor;` — direct property access. An
accessor getter on `constructor.name` (e.g.
`Object.defineProperty(MyClass, 'name', { get: () => 'Spoofed' })`) can therefore spoof
the read. Descriptor-based reading would protect against this case because accessor
descriptors have `value === undefined` rather than invoking the getter. The edit candidate
is to replace `const { name } = constructor` with
`const name = getOwnPropertyDescriptor(constructor, 'name')?.value` and keep the
`isStringValue` narrow that follows. The trade-off is one more `getOwnPropertyDescriptor`
call against tighter spoof protection. Whether the impl should switch is the user's call;
the `.d.ts` doc has been aligned to whatever the impl actually does, so the question can
wait without leaving doc-vs-impl drift behind.
