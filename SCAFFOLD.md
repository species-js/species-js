...
| `docs:sweep`      | JSDoc hygiene (duplicate `@param`, dead `@typedef {import(…)}`) and `.js` exports missing from the sibling `.d.ts` (including `@@throw-safe` marker parity over the declared set, ADR #101). **Structural half only** — the claim sweep takes the round's old wording as arguments and is run by hand             |
| `docs:check`      | typedoc strict validation — broken `{@link}`, undocumented exports, unexported referenced types                                                                                                                                                                                                                   |
| `decisions:check` | An ADR supersession with no reciprocal annotation at its target, leaving the target reading as current when it is not; an `Enforced by:` gate that does not cite the ADR back (R4, #102); a decision-log index row whose date contradicts the ADR (R5)                                                            |
