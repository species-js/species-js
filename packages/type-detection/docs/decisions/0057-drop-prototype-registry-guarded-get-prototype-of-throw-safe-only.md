# 057 — `prototypeRegistry` dropped; `getSafePrototypeOf` is throw-safe-only

**Date:** 2026-06-23

**Context.** The constructor-resolution layer carried three module-scoped `WeakMap`
memoizations — `prototypeRegistry` (behind `getSafePrototypeOf`), `constructorRegistry`,
...
| `getSafePrototypeOf`    | winner   | margin |
| ----------------------- | -------- | ------ |
...
registry on a hot path: `isPromise` calls `getSafePrototypeOf` on every call (for the
...
`getRegisteredPrototype`. Rewrite `getSafePrototypeOf` as throw-safe-only:

```js
export function getSafePrototypeOf(value = null) {
...
- **Behavior unchanged.** `getSafePrototypeOf` returns exactly what it returned before
