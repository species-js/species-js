...
for `doesImplementPromiseContract` (three times, for `then`/`catch`/`finally`). Inlining
three times would duplicate the inspect-without-invoke contract — and any future
...
`isThenable`, three times in `doesImplementPromiseContract`). The name `hasInertMethod`
was chosen over candidates like `hasTrustedMethod` (overloaded — "trusted against what?")
and `hasDataMethod` (spec-precise but obscures the safety frame) because "inert" is
