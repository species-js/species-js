...
| `@/primitive` | [primitive.md](./primitive.md) | Five primitive families: string, number, boolean, bigint, symbol; the boxed-primitive discrimination chain with the local-realm `instanceof` shortcut; three generic floor predicates (`isBoxablePrimitive`, `isNullishPrimitive`, `isPrimitiveValue`) plus the `isBoxedPrimitive` umbrella |
